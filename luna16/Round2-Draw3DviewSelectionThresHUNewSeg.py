...
def ExtractLungRound2(sourceFolder):
            ...
            if len(myslices)>1:
                ...
                if myimage.shape[0]>10:
                    if int(myslices[0].SliceThickness)<=10:
                        ...
                        myimage.shape
                        segmented_lungs_fill = segment_lung_mask(myimage)
                        myimage[segmented_lungs_fill==1]=0
                        segmented_lungs_fill = segment_lung_mask(myimage, True)
                        # xNum = segmented_lungs_fill.shape[0]
                        # segmented_lungs_fill[(xNum-50):(xNum-1),:,:] = 0
                        # segmented_lungs_fill[0:50,:,:] = 0
                        try:
                            print(path1)
                            save_3d(segmented_lungs_fill,path1,threshold=0)
                            plt.close()
                            ToLung_npy(myimage, segmented_lungs_fill, "Old"+path1,threshold=5)
                        except:
                            cv2.imwrite(path1+".png",myimage[0])



sourceFolder = "/data/yeheng/DataProcessing/FromSelectedSampleTonpy/AllRound1_Pickup/Round1NoUse"
ExtractLungRound2(sourceFolder)